...
We exend the reduction with types, as follows:
   ...
   Γ:t  ⇓ρ  Δ:z
becomes:
  Ξ ⊢ (let Γ , in t ⇓ let Δ in z) :ρ A

Then we can prove

if Ξ|Γ ⊢ t : ρA then Ξ|Δ ⊢ z : ρA
where Ξ|Γ ⊢ t :ρ A  ≝   Ξ ⊢ let Γ, x :ρ A = t in x : A (*)

by an easy induction.

(*) this form allows ρ-times computed results to be put-back in the
heap in the shared var rule.
...
The rules are:
  ...
  Ξ, x:ωB ⊢ (let Γ in e ⇓ let Δ in z) :ρ A
---------------------------------------------------------------- shared variable
  Ξ ⊢ (let Γ,x :ω B = e in x  ⇓ let Δ, x :ω B = z in z) :ρ A




  Ξ,x:1B ⊢ (let Γ in e ⇓ let Δ in z) :1 A
----------------------------------------------------- linear variable
  Ξ ⊢ (let Γ,x :1 B = e in x  ⇓  let Δ in z) :1 A



  Ξ ⊢ (let Γ, x :ρq B = e1 in t ⇓ let Δ in z) :ρ A
---------------------------------------------------------- let
  Ξ ⊢ (let Γ in let x :q B = e1 in t ⇓ let Δ in z) :ρ A



  qΨ ⊢ (let Γ in e ⇓ let Δ in c x) :ρ D
  Ξ ⊢ (let Δ in u[x/y] ⇓ let θ in z) :ρ C
------------------------------------------------------------------- case
  (Ξ+qΨ) ⊢ (let Γ in case[q] e of {c y ↦ u } ⇓ let θ in z) : C


  ωΨ ⊢ (let Γ in e ⇓ let Δ in Bang x) :1 Bang A -- clarify!
  Ξ ⊢ (let Δ in u[x/y] ⇓ let θ in z) :ρ C
-------------------------------------------------------------------------- case
  (Ξ+ωΨ) ⊢ (let Γ in case[1] e of {Bang y ↦ u} ⇓ let θ in z) :ρ C
