...
We exend the reduction with types, as follows:
   ...
   Γ:t ⇓ρ Δ:z
becomes:
  ρΞ ⊢ (let Γ in t ⇓ let Δ in z) : A

Then we can prove

if ρΞ ⊢ let Γ in t : A then ρΞ ⊢ let Δ in z : A
...
The rules are:
  ...
  ωΞ,x:ωB ⊢ (let Γ in e ⇓ let Δ in z) : A
---------------------------------------------------------------- shared variable
  ρΞ ⊢ (let Γ,x :ω B = e in x  ⇓ let Δ, x :ω B = z in z) : A




  1Ξ,x:1B ⊢ (let Γ in e ⇓ let Δ in z) : A
----------------------------------------------------- linear variable
  1Ξ ⊢ (let Γ,x :1 B = e in x  ⇓ let Δ in z) : A



  ρΞ ⊢ (let Γ, x :q B = e1 in t ⇓ let Δ in z) : A
---------------------------------------------------------- let
  ρΞ ⊢ (let Γ in let x :q B = e1 in t ⇓ let Δ in z) : A



  ρqΨ ⊢ (let Γ in e ⇓ let Δ in c x) : D
  ρΞ ⊢ (let Δ in u[x/y] ⇓ let θ in z) : C
------------------------------------------------------------------- case
  ρ(Ξ+qΨ) ⊢ (let Γ in case[q] e of {c y ↦ u } ⇓ let θ in z) : C


  1Ψ ⊢ (let Γ in e ⇓ let Δ in Bang x) : Bang A -- clarify!
  ωΞ+ωΨ ⊢ (let Δ in u[x/y] ⇓ let θ in z) : C
-------------------------------------------------------------------------- case
  ω(Ξ+1Ψ+ωΨ) ⊢ (let Γ in case[1] e of {Bang y ↦ u} ⇓ let θ in z) : C
