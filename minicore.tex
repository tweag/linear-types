...
pattern-matching failure simply raise an imprecise exception as
usual. This is equivalent to having exhaustive case expression, with
\verb+error+ as their right-hand side.
...
$\mult{x}$ will only be used\jp{What does ``used'' mean in this
  context? In the algorithm it seems that every occurence of $x$
  refers to $\mult{x}$. Perhaps this is a mistake?} at binding site.
