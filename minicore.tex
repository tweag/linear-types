...
See \fref{fig:typing}.\improvement{Todo: typing rule for
  wildcard pattern.}

Explanation of the case rule. We make sure that every field $x$ is
used appropriately. The scrutinee comes with multiplicity $π$, and
thus each $x$ has to have $πμ$ usages. But each use of the scrutinee
variable $z$ causes $νμ$ usages of $x$. Thus, the constraint
$ρ+νμ=πμ$. (Note: if there is no field, then we're always good; the
tag of the constructor is forced, and thus it does not matter how many
times we use $z$.)

\improvement{Use a named substitution instead of directly $[π_1…π_n]$,
  in the case rule. For clarity.}
\improvement{Define multiplicity equality as a judgement.}
\improvement{Well-typedness of case alternatives as a separate judgement}
...
\newcommand{\caserule}{\inferrule{Γ   ⊢  t  : D~π_1~…~π_n \\
      Δ, z:_{ν_k} D~π_1~…~π_n, x₁:_{ρ_1} A_i, …, x_{n_k}:_{ρ_{n_k}} A_{n_k} ⊢ u_k : C \\
      ρ_1+ν_kμ_1[π_1…π_n]=πμ_1[π_1…π_n]\quad…\quad ρ_{n_k}+ν_kμ_{n_k}[π_1…π_n]=πμ_{n_k}[π_1…π_n]\\
      \text{$c_k : ∀p₁…p_n. A_1 →_{μ_1} … →_{μ_{n_k-1}} A_{n_k} →_{μ_{n_k}} D~p_1~…~p_n$}\\
      \text{for each $1 ⩽ k ⩽ m$}}
