...
\inferrule{ }{ωΓ + x :_1 A ⊢ x : A}\text{variable}

\inferrule{Γ, x :_{π} A  ⊢   t : B}
          {Γ ⊢ λx. t  :  A  →_π  B}\text{abs}

\inferrule{Γ ⊢ t :  A →_π B  \\   Δ ⊢ u : A}
          {Γ+πΔ ⊢ t_π u  :  B}\text{app}

\inferrule{Δᵢ ⊢ tᵢ : Aᵢ \\ \text {$c_k$ is a constructor of $D$ with arguments $Aᵢ$ and weights $πᵢ$}}
          {\sum_i πᵢΔᵢ ⊢ c_k  t₁ … t_n :  D}\text{constructor}

\inferrule{Γ   ⊢  t  : D  \\ Δ, x₁:_{ρπᵢ} Aᵢ, …, x_{n_k}:_{ρπ_{n_k}} A_{n_k} ⊢ u_k : C \\ \text{with each $c_k$ as above}}
          {ρΓ+Δ ⊢ \case[ρ] t {c_k  x₁ … x_{n_k} → u_k} : C}\text{case}


\inferrule{Γᵢ   ⊢  tᵢ  : Aᵢ  \\ Δ, x₁:_{π₁} A₁ …  x_n:_{π_{n}} A_n ⊢ u : C }
