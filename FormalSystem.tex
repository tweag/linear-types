...
We must perform the translation on typed terms in general, because we
need the full info about weights.  Nonetheless, a sub-part of the
syntax does not need the weights, and we just write the translation
rules in compact form, as Launchbury:
\begin{align*}
(λx. t)^* &= λx. (t)^* \\
x^*       &= x \\
(t₁ x )^* &= (t₁)^* x \\
