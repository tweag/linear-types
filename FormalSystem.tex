...
\inferrule{Γ   ⊢  t  : D  \\ Δ, x₁:_{πᵢ} Aᵢ, …, x_{n_k}:_{π_{n_k}} A_{n_k} ⊢ u_k : C \\ \text{with each $c_k$ as above}}
          {Γ+Δ ⊢ \case t {c_k  x₁ … x_{n_k} → u_k} : C}\text{case}
...
% - an input with static weight 1 can have dynamic weight w
