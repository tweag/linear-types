...
\begin{align*}
\mathsf{data} D = c_k  π₁ A₁  …  π_{n_k} A_{n_k}
\end{align*}
...
\inferrule{Γ   ⊢  t  :_{ρ} A  \\ Δ, x₁:_{πᵢρ} Aᵢ, …, x_n:_{π_nρ} A_n ⊢ u_k :_ρ C \\ \text{with $c_k$ as above}}
          {Γ+Δ ⊢ \case t {c_k  x₁ … x_n → u_k} :_ρ C}\text{case}

\inferrule{Γᵢ   ⊢  tᵢ  :_{πᵢρ} Aᵢ  \\ Δ, x₁:_{π₁ρ} A₁ …  x_n:_{π_{nρ}} A_n ⊢  \\ Δ ⊢ u :_ρ C }
...
x^*       &= x \\
(t₁ x )^* &= (t₁)^* x \\
(\case y {c_k  x₁ … x_n → u_k})^* &= \case y {c_k  x₁ … x_n → (u_k)^*} \\
\end{align*}
...
\left(\inferrule{Γ ⊢ t :_ρ  A →_π B  \\   Δ ⊢ u :_{πρ} A}
          {Γ+Δ ⊢ t u  :_ρ  B}\right)^* =
  \flet y =_{π} (u)^* \fin (t)^* y
\end{mathpar}

Constructors are treated similarly. The general case is as follows: \(c  t₁ … t_n = \flet x₁ =_{π_1} t₁,…, x_n =_{π_n} t_n \fin c x₁ …
x_n\) (where \(π_i\) are the weights of the constructor arguments)
As for the application, the translation does not need to bind arguments that are already
...
\begin{mathpar}
\left(\inferrule{Γ   ⊢  t  :_{ρ} A  \\ Δ, x₁:_{πᵢρ} Aᵢ, …, x_n:_{π_nρ} A_n ⊢ u_k :_ρ C}
          {Γ+Δ ⊢ \case t {c_k  x₁ … x_n → u_k} :_ρ C}\right)^* =

          \flet y =_ρ (t)^* \fin \case y {c_k  x₁ … x_n → (u_k)^*}
\end{mathpar}

In $\flet$, the weights must be scaled too:
\begin{mathpar}
\left(\inferrule{Γᵢ   ⊢  tᵢ  :_{πᵢρ} Aᵢ  \\ Δ, x₁:_{π₁ρ} A₁ …  x_n:_{π_{nρ}} A_n ⊢  \\ Δ ⊢ u :_ρ C }
