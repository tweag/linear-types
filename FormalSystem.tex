...
The weight is used to interpret $\flet =1 …$ bindings into bindings on
the proper heap.  Indeed, in ω contexts, $\flet =1 …$ must go on the
...
The function f is a creates some data. When run in a linear context, f
allocates $z$ on the linear heap. When run in an unrestricted context, it
must allocate $z$ on the GC heap. So, its behaviour depends the value of ρ.
