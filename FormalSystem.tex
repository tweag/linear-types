...
\inferrule{Δᵢ ⊢ tᵢ : Aᵢ \\ \text {$c_k$ is a constructor of $D$ with arguments $Aᵢ$ and weights $qᵢ$}}
          {\sum_i qᵢΔᵢ ⊢ c_k  t₁ … t_n :  D}\text{constructor}
...
\inferrule{Γ;Φ : e ⇓_ρ Δ;Ψ : λy.e' \\  Δ;Ψ : e'[x/y] ⇓_{qρ} Θ;Ξ : z}
           {Γ;Φ : e_q x ⇓_ρ Θ;Ξ : z} \text{application}
...
{Γ;Φ : \flet x₁ =_{q₁} e₁ … x_n =_{q_n} e_n \fin e ⇓_ρ Δ;Ψ : z}\text{let}

\text{where $i$ ranges over the subset of indices such that $qᵢρ$ = ω,}

\text{and $j$ ranges over the subset such that $q_jρ$ = 1.}
