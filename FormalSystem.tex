...
As in \textcite{launchbury_natural_1993}, we translate from terms to terms where values are
...
'real' implementation (such as in GHC), one would probably recover
this information on the fly with only lightweight annotations in the terms.
In the interest of a simple presentation, we choose here to instead perform the
translation over the type derivations.

We begin with the trivial cases, which are oblivious to weights, in
the style of Launchbury:
\begin{align*}
(λx. t)^* &= λx. (t)^* \\
x^*       &= x \\
(t₁ x )^* &= (t₁)^* x \\
...
For the case analysis construct, we bind the intermediate results to a variable of the
...
\begin{mathpar}
\left(\inferrule{Γᵢ   ⊢  tᵢ  :_{πᵢρ} Aᵢ  \\ Δ, x₁:_{π₁ρ} A₁ …  x_n:_{π_{n}ρ} A_n ⊢ u :_ρ C }
