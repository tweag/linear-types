...
\title{Extending Haskell with linear types: a core calculus}
...
\maketitle
\begin{abstract}
  \todo{abstract (also improve title)}
\end{abstract}

\section{A calculus of linear types}

We introduce a calculus, which we name \calc{}, to make the design of
the linear type extension precise. The extension strives for seamless
integration into existing Haskell. This is reflected in \calc{}.
...
The best way to think of linear data is data that may not be
controlled by the garbage collector: \emph{e.g.} because they are
...
polymorphism, it is possible that linear data is actually controlled by
